...
public interface ResizableArea<A extends ResizableArea<A>> extends Area<A> {
	...
	class ResizeMouseEventHandler implements EventHandler<MouseEvent> {
	...
	default <T extends BorderPane & IFluentBorderPane<T>> T resizable(T wrapper){
		...
		wrapper
				...
				.addHandler(MouseEvent.MOUSE_DRAGGED, new ResizeMouseEventHandler(getThis(), resizeReference))
				.addHandler(MouseEvent.MOUSE_RELEASED, event -> resizeReference.set(null));
		return wrapper;
	}

	default FluentBorderPane resizableWrapper() {
		return resizable(new FluentBorderPane());
	}
